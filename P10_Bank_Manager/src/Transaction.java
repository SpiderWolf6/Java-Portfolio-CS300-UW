...
/**
 * This class manages individual transactions, including creating and comparing different
 * transactions
 */
public class Transaction implements Comparable<Transaction> {
	...
	public int compareTo(Transaction other) {
		// first, check priority of this transaction and other transaction
	  if(this.priority.ordinal() > other.priority.ordinal()) {
		  ...
		  return -1; 
	        ...
	        // if both priorities are the same, check user balances to compare
		  if(this.getUser().getBalance() > other.getUser().getBalance()) {
		    return 1;
		  }
		  else if(this.getUser().getBalance() < other.getUser().getBalance()) {
		    return -1;
		  }
		  else return 0; // if priorities and balances match, transactions are equal
		}
	}
	...
	public Transaction(Account user, double amount, Type type) {
		if(amount <= 0) {
		  throw new IllegalArgumentException("amount is 0 or less"); // if zero or negative amount, throw error
		}
		// set user, amount, type, and priority as expected
		this.user = user;
		this.amount = amount;
		this.type = type;
		if(type.ordinal() == 2) {
