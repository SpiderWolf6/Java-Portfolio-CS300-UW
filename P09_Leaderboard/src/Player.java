...
public class Player implements Comparable<Player> {
  ...
  public int compareTo(Player other) {
    if(this.score < other.score) {
      return -1; // if lower score, return -1
    }
    else if(this.score > other.score) {
      return 1; // else if higher score, return 1
    }
    else {
      if(this.compareToName(other) < 0) {
        return -1; // if scores are the same but alphabetically lower, then return -1
      }
      else if(this.compareToName(other) > 0) {
        return 1; // if scores are the same but alphabetically higher, then return 1
      }
    }
    return 0;
  }
